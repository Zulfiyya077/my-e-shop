...
const About = () => {
  ...
  const stats = [
    ...
    { icon: Package, value: "10K+", label: "Products Sold", color: "from-[#FFB300] to-[#FF7043]" },
    { icon: Award, value: "500+", label: "5-Star Reviews", color: "from-[#FF7043] to-[#FF6F20]" },
    { icon: TrendingUp, value: "98%", label: "Satisfaction Rate", color: "from-[#FF6F20] to-[#FFB300]" }
  ];
  ...
  return (
    <div className="min-h-screen bg-[#FFF3E0]">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          {stats.map((stat, index) => (
            <motion.div
              ...
              className="bg-white rounded-2xl shadow-xl p-6 text-center border-2 border-[#FF6F20]"
            >
              <div className={`inline-block p-4 bg-gradient-to-r ${stat.color} rounded-xl mb-4`}>
                <stat.icon size={32} className="text-white" />
              </div>
              <h3 className="text-4xl font-black mb-2 text-[#4A4A4A]">
                {stat.value}
              </h3>
              <p className="text-[#FF6F20] font-semibold">{stat.label}</p>
            </motion.div>
          ))}
        </div>
      </motion.section>
        ...
        <div className="grid lg:grid-cols-2 gap-12 items-center">
          <motion.div variants={itemVariants} className="space-y-6">
            <p className="text-gray-600 leading-relaxed text-lg">
              Welcome to <span className="font-bold text-[#FF6F20]">ZIpTech</span>! We are dedicated to providing the best electronic products with top-notch customer service. Our mission is to make shopping easy and enjoyable for everyone.
            </p>
            <p className="text-gray-600 leading-relaxed text-lg">
          ...
          <motion.div 
            ...
            className="relative"
          >
            <div className="relative rounded-2xl overflow-hidden shadow-2xl border-4 border-[#FF6F20]">
              <img 
                src="https://images.unsplash.com/photo-1522071820081-009f0129c71c?w=800&q=80" 
                alt="Team" 
                className="w-full h-96 object-cover"
              />
              <div className="absolute inset-0 bg-gradient-to-t from-[#FF6F20]/60 to-transparent" />
            </div>
            {/* Floating Badge */}
            <motion.div
              animate={{ y: [0, -10, 0] }}
              transition={{ duration: 3, repeat: Infinity }}
              className="absolute -bottom-6 -right-6 bg-gradient-to-r from-[#FF6F20] to-[#FFB300] text-white p-6 rounded-2xl shadow-2xl border-4 border-white"
            >
        ...
        <div className="max-w-6xl mx-auto px-6">
          ...
          <div className="grid md:grid-cols-3 gap-8">
            {values.map((value, index) => (
              ...
              >
                <div className="bg-[#FFF3E0] rounded-2xl p-8 shadow-lg group-hover:shadow-2xl transition-all border-2 border-[#FF6F20]">
                  <div className={`inline-block p-4 bg-gradient-to-r ${value.gradient} rounded-xl mb-6`}>
                    <value.icon size={32} className="text-white" />
                  </div>
                  <h3 className="text-2xl font-bold mb-4 text-[#4A4A4A]">{value.title}</h3>
      ...
      >
        <motion.div 
          variants={itemVariants}
          className="bg-gradient-to-r from-[#FF6F20] via-[#FFB300] to-[#FF7043] rounded-3xl p-12 text-white relative overflow-hidden border-4 border-white shadow-2xl"
        >
          {/* Background Pattern */}
          ...
          <div className="relative z-10 text-center max-w-3xl mx-auto">
            ...
            <motion.p 
              variants={itemVariants}
              className="text-xl leading-relaxed opacity-95"
            >
              ...
              >
                <div className="relative inline-block mb-4">
                  <motion.img
                    ...
                    src={member.img}
                    alt={member.name}
                    className="w-32 h-32 rounded-full object-cover shadow-xl border-4 border-[#FF6F20]"
                  />
                  <div className="absolute inset-0 rounded-full bg-gradient-to-r from-[#FF6F20] to-[#FFB300] opacity-0 group-hover:opacity-20 transition-opacity" />
                </div>
                <h3 className="text-xl font-bold text-[#4A4A4A]">{member.name}</h3>
                <p className="text-[#FF6F20] font-semibold">{member.role}</p>
              </motion.div>
            ))}
          </div>
        </div>
        ...
        <div className="grid md:grid-cols-3 gap-8">
          {[
            ...
            <motion.div
              ...
              variants={itemVariants}
              whileHover={{ scale: 1.05, y: -5 }}
              className="bg-white rounded-2xl p-8 shadow-lg text-center border-2 border-[#FF6F20]"
            >
              <div className={`inline-block p-4 bg-gradient-to-r ${contact.color} rounded-xl mb-4`}>
                <contact.icon size={28} className="text-white" />
              </div>
              <h3 className="text-xl font-bold mb-2 text-[#4A4A4A]">{contact.label}</h3>
              <p className="text-[#FF6F20] font-semibold">{contact.value}</p>
            </motion.div>
          ))}
        </div>
      </motion.section>
